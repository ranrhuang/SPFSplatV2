...
def render_cameras_es(context_extrinsics, target_extrinsics, batch: dict, resolution: int) -> Float[Tensor, "3 3 height width"]:
    # Define colors for context and target views.
    # num_context_views = batch["context"]["extrinsics"].shape[1]
    # num_target_views = batch["target"]["extrinsics"].shape[1]
    ...
    color = torch.ones(
        ...
        dtype=torch.float32,
        # device=batch["target"]["extrinsics"].device,
        device=target_extrinsics.device
    )
    color[num_context_views:, 1:] = 0
    ...
    return draw_cameras(
        resolution,
        torch.cat(
            (context_extrinsics[0], target_extrinsics[0])
        ),
        torch.cat(
            (batch["context"]["intrinsics"][0], batch["target"]["intrinsics"][0])
        ),
